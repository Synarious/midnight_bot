...
const { getWowGuildSettings } = require('../../data/automodSettings');

// ==================== DEFAULT CONFIGURATION ====================
// These are fallback values if not configured in database
const DEFAULT_GATE_ROLE_ID = '1442686627481911356';
const DEFAULT_WOW_MEMBER_ROLE_ID = '1129313754778194050';
const DEFAULT_WOW_WELCOME_CHANNEL_ID = '713412331761172506';
const DEFAULT_WOW_LOG_CHANNEL_ID = '1443006368897306728';
const DEFAULT_REALM_SLUG = 'moon-guard';
const DEFAULT_ONBOARDING_CODE = 'GB16';
...
async function handleButton(interaction) {
    ...
    try {
        // Check if this guild has WoW guild onboarding enabled
        const settings = await getWowGuildSettings(interaction.guild.id);
...
async function handleModal(interaction) {
    ...
    try {
        ...
        const onboardingCode = settings.onboarding_code || DEFAULT_ONBOARDING_CODE;
        if (codeRaw.toLowerCase() !== onboardingCode.toLowerCase()) {
        ...
        // Add WoW Member role
        const memberRoleId = settings.member_role_id || DEFAULT_WOW_MEMBER_ROLE_ID;
        if (memberRoleId && !member.roles.cache.has(memberRoleId)) {
            try {
                await member.roles.add(memberRoleId);
                console.log(`[wowGuild] Added WoW Member role ${memberRoleId} to ${interaction.user.tag}`);
            } catch (roleErr) {
        ...
        const welcomeChannelId = settings.welcome_channel_id || DEFAULT_WOW_WELCOME_CHANNEL_ID;
        if (welcomeChannelId) {
        ...
        const logChannelId = settings.log_channel_id || DEFAULT_WOW_LOG_CHANNEL_ID;
        const realmSlug = settings.realm_slug || DEFAULT_REALM_SLUG;
        if (logChannelId) {
            ...
            if (logChannel) {
                const logEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: 'Realm', value: realmSlug, inline: true },
